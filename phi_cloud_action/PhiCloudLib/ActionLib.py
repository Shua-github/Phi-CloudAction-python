...
from .other import get_info_dir
from .._utils import b30, savesHistory
...
def rate(real_score: int, tot_score: int = 1000000, fc: bool = False) -> str:
    """
    排名评级喵

    参数:
        real_score (int): 玩家的实际分数喵
        tot_score (int): 总分喵,默认为1000000
        fc (bool): 是否FC喵
...
def readDifficultyFile(
    path=str(get_info_dir() / "difficulty.tsv"),
) -> Dict[str, List[float]]:
...
# 读取info文件
def readInfoFile(path=str(get_info_dir() / "info.tsv")) -> Dict[str, List[str]]:
    if path.endswith(".tsv"):
        return readFile.info_tsv(path)
    elif path.endswith(".csv"):
        return readFile.info_csv(path)
    else:
        raise ValueError("info文件应该是.tsv或.csv")
...
class readFile:
    ...
    # 读取tsv格式的info文件
    @staticmethod
    def info_tsv(path: str) -> Dict[str, List[str]]:
        """
        读取info文件 (TSV格式)
        ...
        返回:
            (dict[str, list[str]]): info数据。以歌曲id为键，值为字符串列表
        """
        info_list = {}

        with open(path, encoding="UTF-8", newline="") as f:
            reader = csv.reader(f, delimiter="\t")  # 使用tab分隔符
            for row in reader:
                song_id = row[0]
                value = [str(value) for value in row[1:]]  # 转为字符串
                info_list[song_id] = value  # 将歌曲id和值存入字典

        return info_list  # 返回字典

    # 读取csv格式的info文件
    @staticmethod
    def info_csv(path: str) -> Dict[str, List[str]]:
        """
        读取info文件 (CSV格式)
        ...
        with open(path, encoding="UTF-8", newline="") as f:
            reader = csv.reader(f)  # 默认使用逗号分隔符
            next(reader)  # 跳过标题行

            for row in reader:
                song_id = row[0]  # 获取歌曲ID
                value = [str(v) for v in row[1:]]  # 获取后面的值并转为字符串
                info_list[song_id] = value
    ...
    def difficulty_tsv(path: str) -> Dict[str, List[float]]:
        ...
        返回:
            (dict[str, list[float]]): 难度数据。以歌曲id为键，值为浮动值列表
        """
        difficulty_list = {}
        ...
        with open(path, encoding="UTF-8", newline="") as f:
            ...
            for row in reader:
                song_id = row[0]
                diff = [float(value) for value in row[1:]]  # 转换为float
                difficulty_list[song_id] = diff  # 将歌曲id和难度值存入字典

        return difficulty_list  # 返回字典
    ...
    def difficulty_csv(path: str = "./info/difficulty.csv") -> Dict[str, List[float]]:
        ...
        with open(path, encoding="UTF-8") as f:
            ...
            for row in reader:
                song_id = row[0]  # 获取歌曲ID
                diff_values = [float(x) for x in row[1:] if x]  # 忽略空值并转为float
...
def countRks(record_data: dict, difficulty: Dict[str, list], countRks: bool = True):
    """
...
def getB30(records: dict, difficult: dict, b_num: int = 27, p_num: int = 3) -> b30:
    """
    获取b30喵
    ...
    for song in record.items():  # 遍历所有歌曲记录喵
        for song_record in song[1].items():  # 遍历每首歌的所有难度记录喵
            ...
            # 检查歌名是否存在于difficult字典中
            if song[0] not in difficult:
                logger.warning(f"歌名 {song[0]} 不在难度字典中喵！")
                continue

            song_difficulties = difficult[song[0]]

            # 检查难度是否在diff_list中
            if song_record[0] not in diff_list:
                logger.warning(f"难度 {song_record[0]} 不在diff_list字典中喵！")
                continue

            # 使用song_record[0]的索引访问difficulty
            try:
                difficulty = song_difficulties[diff_list[song_record[0]]]
            except IndexError:
                logger.warning(f"难度 {song_record[0]} 在歌名 {song[0]} 的难度列表中越界喵！")
                continue
            
            song_record[1]["difficulty"] = difficulty
            song_record[1]["rks"] = ((song_record[1]["acc"] - 55) / 45) ** 2 * difficulty
...
def readSaveHistory(sessionToken: str) -> savesHistory:
    ...
    userHistory_path = f"saveHistory/{sessionToken}/user.json"
    SaveDictHistoty_path = f"saveHistory/{sessionToken}/SaveDictHistory.json"
    if (
        not exists(recordHistory_path)
        or not exists(summaryHistory_path)
        or not exists(userHistory_path)
        or not exists(SaveDictHistoty_path)
    ):
    ...
    user: dict = readFile.json(userHistory_path)
    save_dict: dict = readFile.json(SaveDictHistoty_path)
    return savesHistory(
        record=recordHistory, summary=summaryHistory, user=user, save_dict=save_dict
    )
...
def checkSaveHistory(
    ...
    if not exists(f"saveHistory/{sessionToken}/user.json"):
        recordHistory = {}
        logger.info("对应sessionToken的user历史文件不存在喵！已创建喵！")

    else:
        with open(
            f"saveHistory/{sessionToken}/user.json",
            "r",
            encoding="utf-8",
        ) as file:
            user = loads(file.read())
    ...
    if not summary["checksum"] in checksumHistory:
        ...
        if differentRecord != []:
            ...
            with open(
                f"saveHistory/{sessionToken}/SaveDictHistory.json",
                "w",
                encoding="utf-8",
            ) as file:
            ...
            with open(
                ...
                file.write(dumps(user, indent=4, ensure_ascii=False))
